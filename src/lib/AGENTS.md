...
## Purpose
Core business logic library for Saju (사주) calculations, AI prompt construction, classical reference data, and internationalization. All files are pure TypeScript with no React dependencies.
...
| `saju-prompt.ts` | System prompt for GPT-4o — defines `SAJU_SYSTEM_PROMPT` with detailed instructions for the AI persona (40-year 명리학 master), interpretation rules, and `UserData` interface |
...
| `saju-prompt-builder.ts` | Builds structured user prompts — takes `SajuResult` + `UserData`, computes 십성/신살/충, and generates prompt text with all Saju data for GPT-4o |
| `saju-references.ts` | RAG-style reference library — ~130 classical quotes organized by topic (personality, wealth, love, career, health, compatibility, timing, general) from 적천수, 자평진전, 궁통보감, 연해자평, 삼명통회 |
...
### External
- None — all files are pure TypeScript with zero external dependencies
