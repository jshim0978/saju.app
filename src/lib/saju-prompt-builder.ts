...
export function buildSajuPrompts(sj: SajuResult, ohCount: Record<string, number>, userData: UserData): string[] {
  ...
  const rules = '=== 규칙 ===\n' +
    ...
    '3. [키워드] 로 시작\n' +
    '4. ⚠️ 각 섹션은 반드시 최소 1000자(약 30줄) 이상! 500자 미만이면 무조건 탈락! 한 섹션 안에서 [키워드]소제목을 2-3개까지만 쓰고, 각 소제목 아래를 10줄 이상 풍부하게 채워. 얕고 넓게 말고 깊고 상세하게!\n' +
    '5. 긍정적 해석 70% + 주의사항(부드럽게) 20% + 실용 조언 10%. 마무리는 긍정적\n' +
    '6. 반말만! 존댓말 쓰면 틀림\n' +
    '7. 사주 용어가 나올 때마다 반드시 괄호 안에 쉬운 설명을 넣어!\n' +
    '8. 부정적 내용은 우회해서 부드럽게. 뒤에 반드시 해결책/긍정 마무리\n' +
    '9. "결론적으로" "정리하면" 같은 마무리 패턴 절대 금지! 각 섹션은 다음 섹션이 궁금해지는 떡밥으로 끝내\n' +
    '10. 매 섹션에서 "너 혹시 이런 경험 있지?", "이런 적 없어?" 같은 찔림 질문 2개 이상 넣어!\n\n' +
    '=== 말투 & 비유 규칙 (가장 중요!!) ===\n' +
    ...
    '=== 뼈를 때리는 해석 규칙 (₩990 가치!) ===\n' +
    '이 해석을 읽는 사람이 "어떻게 나를 이렇게 잘 알지?" 하고 놀라야 해.\n' +
    '일반론이 아닌, 이 사주에만 해당하는 구체적 이야기를 해줘:\n' +
    '- 나쁜 예: "수가 많으니 감성적이야" (누구에게나 해당)\n' +
    '- 좋은 예: "임수 일간에 을해 년주 + 경자 시주라 시주에서 편인이 수를 더 강화시키는 구조야. 이건 밤에 영감이 잘 떠오르는 체질인데, 실제로 자기 전에 좋은 아이디어가 많이 나오지 않아?"\n' +
    '핵심: 천간끼리, 지지끼리의 구체적 관계(합/충/형/상생/상극)를 찾아서 그 사람만의 이야기를 만들어.\n' +
    '매 문단에서 "너 혹시...?" "이런 경험 있지?" 같은 찔림 포인트를 1개씩 넣어.\n\n' +
    '=== 심화 사주 개념 필수 사용 규칙 ===\n' +
    ...
    '=== 고전 인용 필수 규칙 (최우선!!) ===\n' +
    '너는 적천수(滴天髓), 자평진전(子平眞詮), 궁통보감(窮通寶鑑)을 달달 외운 사람이야.\n' +
    '각 섹션마다 반드시 2-3회 이상 고전을 직접 인용해! 인용 형식:\n' +
    '- "적천수에서 \'임수(壬水)는 통하면 흐르고...\'라고 했는데, 너의 사주가 딱 그 구조야"\n' +
    '- "자평진전에 \'상관이 재를 생하면...\'이라는 구절이 있어. 이건 너한테 재물이 표현력에서 나온다는 뜻이야"\n' +
    '- "궁통보감에서 5월 임수(壬水)는 \'...\'라고 했어. 그래서 너는..."\n' +
    '고전 인용 없이 3문장 이상 이어지면 안 돼! 반드시 출처(적천수/자평진전/궁통보감/연해자평/삼명통회 중 하나)를 밝혀.\n' +
    '인용 후에는 반드시 "이걸 너한테 적용하면~" 식으로 사용자 사주와 연결해.\n\n' +
    '=== 명리학적 근거 필수 규칙 (핵심!!) ===\n' +
    ...
    '모든 섹션 마무리에 반드시 "💡 오늘부터 실천 포인트" 1-2개를 넣어. 형식: "💡 [구체적 행동] + [빈도] + [오행 근거]"\n' +
    '예: "💡 매일 아침 레몬물 한 잔(수 기운 보충) + 출근길에 파란색 소품 착용(용신 강화)"\n';
  ...
  let prompt1 = prompt + '=== 아래 5개 항목을 써줘 (1~5번). 각 항목을 깊고 풍부하게! ===\n\n';
  let n = 1;
  ...
  prompt1 += getRelevantRefs({ dayMaster: ds, topics: ['personality', 'wealth', 'career'] });
  prompt1 += getTerminologyPromptBlock(userData.lang === 'en' ? 'en' : 'ko');
  prompt1 += rules;

  // Part 2: Relationships, Health, Family (sections 5-10)
  let prompt1b = prompt + '=== 아래 6개 항목을 써줘 (5~10번). 각 항목을 깊고 풍부하게! ===\n\n';
  if (isMarried) {
    prompt1b += '##' + n++ + '.부부 관계 & 배우자 분석## 배우자궁(' + JJ[sj.dBranch] + ') 기반 배우자의 성향과 부부 사이 역학 분석. 배우자와의 궁합 포인트, 갈등 요인과 해결법. 부부 사이가 좋아지는 시기/조심할 시기(대운/세운 기반). 배우자에게 어떤 식으로 표현하면 좋은지. 권태기 극복법.\n';
  } else {
    prompt1b += '##' + n++ + '.연애 & 인연의 지도## 배우자궁(' + JJ[sj.dBranch] + ') 기반 미래 반쪽의 느낌과 주의점 + 사랑 타이밍(이성운 강한 시기) + 매력 무기(' + (shinsalStr !== '없음' ? shinsalStr : '사주적 매력 요소') + '). 연상/연하 분석(인성/식상/관성 구조 기반). 누가 먼저 다가가는 게 좋은 타입인지.\n';
  }
  prompt1b += '##' + n++ + '.나에게 좋은 사람 & 주의할 사람## [주의: 이 섹션에서는 성격/멘탈/내면 분석 금지. 오직 대인관계와 인연 분석만 할 것] 한 곳에 모아서 총정리:\n' +
  ...
  prompt1b += '##' + n++ + '.건강 리포트## 오행 균형과 조후(사주 온도)로 취약 장기(' + (dayOh==='목'?'간/눈':dayOh==='화'?'심장/혈관':dayOh==='토'?'위/소화기':dayOh==='금'?'폐/호흡기':'신장/방광') + ') 분석 + 관리법 + 맞는 취미/운동 3가지(오행 근거). 오행 부족/과다에 따른 일반적인 건강 경향성 설명. 양인살/백호살 등 신살이 있으면 그 의미 설명. [중요: 구체적인 사고 년도 예측은 하지 마. 특정 질병 진단이나 의학적 조언도 금지. 끝에 반드시 "※ 이것은 명리학적 관점의 건강 경향성이며, 의학적 진단이나 조언이 아닙니다." 포함]\n';
  prompt1b += '##' + n++ + '.가정 & 가족관계## 년주(조상궁) → 월주(부모궁/형제궁) → 일지(배우자궁) → 시주(자녀궁)으로 가정 전체를 분석. 부모와의 관계(월주-일주 충/형 여부), 형제 관계(비겁 구조), 가족 갈등 해소법, 가족 여행 좋은 방위/계절.\n';
  prompt1b += '##' + n++ + '.자녀운 & 부모 스타일## 시주와 식상/관성 근거로: 자녀복(시주 12운성), 자녀가 들어오기 좋은 시기(년도/나이, 통변으로 식상운이 오는 해), 아이 예상 성향(시주 오행), 어떤 부모 스타일이 맞는지.\n';
  prompt1b += '##' + n++ + '.지금 나의 인생 챕터## 현재 나이(' + userAge + '세, ' + lifeStage + ') 대운 분석. 12운성으로 지금 에너지 상태, 이 대운이 인생에서 어떤 의미인지. 현재 대운의 천간/지지가 원국과 어떻게 작용하는지 분석하고, 지금 시기에 집중해야 할 것과 내려놓아야 할 것을 알려줘.\n\n';
  prompt1b += getRelevantRefs({ dayMaster: ds, topics: ['love', 'health', 'general'] });
  prompt1b += getTerminologyPromptBlock(userData.lang === 'en' ? 'en' : 'ko');
  prompt1b += rules;

  // Part 3: Future predictions (sections 11-17)
  let prompt2 = prompt + '=== 아래 7개 항목을 써줘 (11~17번). 각 항목을 깊고 풍부하게! ===\n\n';
  ...
  if (isEn) {
    ...
    prompt1 += engReminder;
    prompt1b += engReminder;
    prompt2 += engReminder;
  }

  return [prompt1, prompt1b, prompt2];
}
